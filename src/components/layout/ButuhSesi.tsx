...
export default function ButuhSesi({ children }: { children: React.ReactNode }) {
  ...
  const multipleActive = sesiAktifList.length > 1;
  ...
  // C-08: gate global juga blokir MULTIPLE active (sebelumnya lolos; hanya halaman meja yang guard).
  if (multipleActive) {
    const jadwalUrlMulti = getRoleMenuPath("/posyandu", currentUser?.peran || "kader");
    return (
      <div className="p-4 sm:p-8 max-w-2xl mx-auto my-8">
        <div className="bg-white rounded-3xl border border-rose-200 shadow-sm p-8 text-center space-y-6">
          ...
          <div className="space-y-2">
            <h2 className="text-xl sm:text-2xl font-black text-gray-900">Terdeteksi Lebih dari Satu Sesi Aktif</h2>
            <p className="text-sm text-gray-500 max-w-md mx-auto leading-relaxed">
              Alur 5 Meja diblokir agar data tidak tercampur antar sesi. Tutup/akhiri sesi ganda di halaman Jadwal Posyandu.
            </p>
          </div>
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              to={jadwalUrlMulti}
              className="w-full sm:w-auto px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-bold rounded-xl text-sm shadow-md shadow-sky-600/20 transition flex items-center justify-center gap-2"
            >
